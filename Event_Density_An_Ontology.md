...
The degree to which a region generates, sustains and transmits events is its **participation**. Event Density is the local rate of participation — the amount of becoming in a region.
...
Two events are near each other only in the sense that they integrate each other's becoming. That is participation adjacency: the precursor of spatial proximity, and not itself geometric.
...
Once time is accumulated becoming and space is stable relations, spacetime appears as their synthesis: the smooth, continuous approximation that emerges when granular activity is coarse-grained over vast numbers of events.

The transition from events to manifolds is the transition from thin to thick participation; from uncommitted to committed structure; from relational adjacency to geometric distance; from commitment order to a temporal dimension; from gradients to curvature.
...
Below that threshold, events are too sparse to maintain stable gradients, and the system becomes thin, indeterminate, quantum. Above it, gradients persist, identities form, and classical behaviour emerges.
...
**Decoherence** is the moment a thin system re-enters participation. When it meets a high-ED environment, event production rises, gradients form, relational structure stabilises, and the system becomes part of a definite causal chain. It is not a collapse, not a branching, not a hidden-variable update. It is a thin region becoming thick enough to behave classically.

**Measurement** is forced participation. A measuring device is a high-ED system: dense production, strong gradients, persistent structures. When it meets a low-ED system the outcome is not mysterious — the thin system is absorbed into a thick attractor, events are produced, and a definite causal chain is established.

**Entanglement** is non-individuation in a low-ED regime. When a high-ED system splits into two ultra-thin fragments, the pair inherits a single participation constraint. Because low-ED systems generate almost no events, their internal updating is effectively suspended: the fragments do not diverge, because nothing happens to them. When decoherence forces both back into participation, they resolve in a way that satisfies the inherited constraint. No signal is exchanged. Nothing propagates.
...
At the micro-scale there is no manifold, no metric, no dimensionality, no global symmetry and no conserved quantity. **Events obey constraints, not laws.**
...
**Fields** are stable participation modes that persist under coarse-graining. **Particles** are stable commitment motifs — repeating, self-reinforcing patterns of committed events. A particle is not a thing. It is a pattern.
...
This was argued first and then measured. On a line and on a flat grid, the coupling needed to hold the clocks together rises without limit as the pattern grows. On a three-dimensional grid and on a random web it rises at most very slowly, with a large majority of clocks staying locked. A flat triangular sheet with the same six connections per event as the three-dimensional grid still fails, so this reads the dimension and not the number of connections.

"""Attempt 9's growth tick with two changes, both switchable (D11; note 9).

Everything else is attempt 8's port unchanged, imported from ../../ED_Attempt_08/model (a closed
record, read and never edited).

1. OFFSPRING (D11, Allen: "mostly exactly one child"). With probability q an event draws its number
   of children from the old spread (geometric with mean mu, which A7 C1 took from CDT to check the
   causal machinery); otherwise it has exactly one. The average stays at one where the budget is at
   one, so Budgeted Causality's balance point is unchanged; its pull toward balance is weakened by q.
   q = 1 reproduces attempt 8 exactly.
2. FLIP RATE. The paired cut-and-rejoin sweep makes flip_frac x V / 2 attempts a tick instead of V/2.
   flip_frac = 1 reproduces attempt 8. It is here only to separate the two sources of churn.
"""
import os
import sys
import time
...
@njit(cache=True)
def tick9(S, rs, alpha, lam, pool, s_max, cap, q, flip_frac, out, kids_out, abs_out, tm):
...
class Slice9P(Slice3P):
    """Slice3P with attempt 9's tick: q (share drawing from the old spread) and flip_frac."""

    def tick9(self, alpha, lam, pool, q=1.0, flip_frac=1.0, s_max=None, cap=None):
        S = self.S
        ...
        pool = tick9(S, S.rs, float(alpha), float(lam), int(pool), sm, int(cap), float(q), float(flip_frac),
                     out, kids, absd, self.phases)
        if S.ctr[C_OVER]:
            raise RuntimeError("capacity overflow in tick9: code %d" % int(S.ctr[C_OVER]))

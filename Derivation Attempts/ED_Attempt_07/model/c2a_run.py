"""C2a runs (note 6, D7; IMPLEMENTATION_NOTES.md, C2a section).

Expected results, written down before the first run (note 6):
  ...
  E2  reading B, k = 0.5, 1, 1.5, both L* (100, 200): >= 19 of 20 seeds survive;
      median mean offspring (slices T/2..T) in [0.98, 1.02]; median mean slice
      length within +-10% of L*.
...
Exit rule (note 6, D7): pass on E1, E2, E5 records "the balance self-organizes
from ED's budget passed forward and conserved: consistent, not derived; k and L*
are knobs; space settles as a fixed-size tube, and growth would need budget
creation (the cosmic excess, inherited)"; E3/E4 not as expected recorded as
stability findings, not blocking; E2 or E5 failing records "balance put in";
...
def main():
    ...
    for reading, k, Ls in settings:
        ...
        for s in range(20):
            ...
            if reading == "B" and not g["budget_ok"]:
                e1 = False
            mo, ml = balance_stats(g["lengths"], T)
        ...
        lines.append(f"reading {reading} k={k} L*={Ls}: survived {surv}/20 (died {sum(r['status']=='died' for r in rows)}, ran away {sum(r['status']=='ran away' for r in rows)}); median mean offspring {mo:.4f}; median mean length {ml:.1f} ({ml/Ls:.3f} of L*)")
    ...
    for name, v in (("E1 structure exact, budget conserved", e1), ("E2 reading B balance, k=0.5,1,1.5", e2),
    ...
    if not e1:
        ...
        verdict = ("PASS: the balance self-organizes from ED's budget passed forward and conserved: consistent, not derived; "
                   "k and L* are knobs; space settles as a fixed-size tube, and growth would need budget creation (the cosmic excess, inherited).")
    else:
        verdict = "Balance put in (E2 or E5 not met); go on to slice dimension with the balance as an input."
    lines.append("EXIT: " + verdict)
    lines.append(f"total {time.perf_counter()-t0:.0f} s")
    print("\n".join(lines[-7:]))
    with open("c2a_run1.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(lines) + "\n")
    with open("c2a_run1.json", "w", encoding="utf-8") as f:

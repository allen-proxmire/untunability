"""C2b run 2: revise and retest (note 10, C33; D13).

Revisions (recorded): R1 sigma = 0.0005 (run 1: 0.002), so every run stays in
the linear regime the calculation covers (run 1's largest neighbour difference
0.884 scales to about 0.22); R2 10 fresh seeds, 3-12 (run 1: 0-2), for the 1D
seed-to-seed scatter. Random-slice seeds 1000 + 10*seed + redraw as in note 9;
each run's slice seed is stored and duplicates at the same (d, n) are reported.
Everything else is unchanged from note 9: sizes, update, readings, ranges, E1-E5,
exit rule, tilt reading (reported only). E1's rate-spread limit is 0.01*sigma
with the new sigma.

Expected results, written down before the first run (note 9, unchanged):
...
Exit rule (note 9):
  ...
  Exponents don't separate (gaps < 0.3 in both G and R): "The floor isn't seen
    in ED's causal pattern"; take stock of road C.
  E5 not as expected: recorded, not blocking.
  E1 fails: code bug or regime problem (recorded).
Resumable: one JSON per run in c2b_runs2/; finished runs are skipped.
"""
import json
import os
import sys
...
def do_job(job):
    ...
    res["slice_seed"] = None if kind == "G" else 1000 + 10 * seed + sl["redraws"]
    ...
    for fresh in (False, True):
        for kind in ("G", "R"):
            for d in (1, 2, 3):
                ...
                redraws = [runs[(d, n, kind, fresh, s)]["redraws"] for n in ns for s in SEEDS]
                extra = ""
                if not fresh:
                ...
                Wmed = [float(np.median([runs[(d, n, kind, fresh, s)]["W"] for s in SEEDS])) for n in ns]
...
def main():
    ...
    print("\n".join(lines))
    with open("c2b_run2.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(lines) + "\n")

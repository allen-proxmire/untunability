"""C3f runs (note 27, C92, with the D41 threshold revision; IMPLEMENTATION_NOTES.md, C3f section). T = 150.

Expected results, written down before the first run (note 27; low confidence except E0, E1):
  Shape at a size (median of seeds), against the flat calibration FC at that size:
    too small to measure: slice d_H undefined, or diameter < 8 -> no shape is assigned;
    crumpled: mean degree >= 2 x flat mean (26.8), or tetrahedra per event >= 2 x 5.699;
    hyperbolic: exponential-growth flag set in both seeds;
    branched: neck strain >= 3x FC's, or d_H < 2.3, or d_s < 2.0;
    flat 3D: none of these, d_H in [2.5, 3.5] and d_s in [2.3, 3.7].
  Settled: median |d_H(T) - d_H(3T/4)| <= 0.15 and median relative change of mean degree <= 20%.
  E0  calibration gate: FC flat 3D at both sizes, RC not; growth runs do not start if it fails.
  E1  structure exact every tick; children's budget to 1e-9; links + pool == B_L exactly; no event over
      the ceiling; flips never change the link count; event count within +-10% of the start in A and D.
  E2  B and C hold their event counts within +-10% too.
  E3  B's grown spacetime mass dimension in [3.3, 4.7] with at least three usable radii.
  E4  D's grown spacetime not in that range.
  E5  every setting measurable at both sizes (slice d_H defined and diameter >= 8).
Exit rule (note 27):
  ...
  E1 or E2 fails only on the event count: "The counts still drift under <settings>", with the per-tick
    bookkeeping naming the cause.
  Then: B's spacetime in range and D's not: "With the link balance, paired cut-and-rejoin and sync as a
    condition, ED's growth gives a four-dimensional pattern at the sizes run: consistent, not derived; the
    density, the ceiling, the sync threshold and the strengths are knobs." A also in range: the same, with
    sync not needed. D in range: "Growth alone gives a four-dimensional pattern at these sizes." None in
    range with counts holding: "ED's local weights don't select smooth geometry against entropy; the wall
    is the ensemble, not the meanings." Runs unmeasurable: "Too small to measure at these sizes."
Sync threshold: s_max = 1.5 x the flat calibration's largest link strain at that size (D41, from C94).
Resumable: one JSON per finished job in c3f_runs/; state saved every 10 ticks, except during the window
...
def main():
    ...
    if not ok:
        ...
        print(lines[-1])
        open("c3f_run1.txt", "w", encoding="utf-8").write("\n".join(lines) + "\n")
        return

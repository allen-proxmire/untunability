...
*Draft 1, 2026-09-14. Written under the exit rule (RD52, RD55): three frozen attempts to make ED fix its one missing ingredient failed, so ED is written up for what it honestly is. Every claim below points to the ledger (`01_Ledger/Claims.md`, 301 claims; `Assumptions.md`, RD1–RD55 and D1–D18). The 36 computational checks rerun with one command (`01_Ledger/checks.txt`).*
...
**What ED is not (yet).** ED makes no new prediction that could be tested. Every fit matched a measurement rather than predicting one, and every distinctive variant ED tried either failed against data or failed to fix itself from ED's primitives. Those failures are recorded here in full.

**Why it may still be worth reading.**
- **A careful, falsification-first attempt.** It develops an irreversible-substrate picture of physics with frozen predictions, recorded misses and literature checks at every step.
...
**Verdict.** ED's gravity is general relativity in budget language. Each step matched a measurement; none predicted one.
...
|---|---|---|
| Interference, partial which-path marks (D² + V² ≤ 1), the quantum eraser, reversible hinging | Rule versions 1 and 3 (the rebuilt draw), frozen predictions | C127–C129, C291 |
...
### 4.3 Can ED fix the missing ingredient? Three frozen attempts (the exit rule)
...
| **Coherence readings as the source of the standing phase** | Three frozen attempts failed | C296–C303 |
...
| **Objective-collapse models** (GRW, Penrose) | ED is *not* one: it has no spontaneous collapse, and so predicts no deviations from quantum mechanics |
...
- **Predictions frozen before every run,** in the check script or spec, with confidence levels.
- **Every miss recorded.** That includes Claude's own design errors and test-code bugs, alongside the physics misses. Since 2026-09-13 the track record includes runs of 10 of 10 and 8 of 8 right, and misses such as 4 of 10 and 4 of 9 (`Stock_Take.md`).
- **Literature checks before claims.** Sources are marked by how they were checked (abstract, paper text, or search listing only).
- **Settings chosen to make something work are labelled as tuned,** and never counted as predictions.
...
*Draft 1: for Allen's review. Wording, emphasis, and whether to include the philosophy section are his calls.*
